...
class TuckBoxGenerator:
    def __init__(self, width, height, depth, fname=None, 
                 sideImage=None,
                 frontImage=None,
                 backImage=None,
                 endImage=None,
    ...
    def drawEnd(self,isTop=False, isGlue=False):
        if self.fillColour:
            ...
            self.canvas.rect(0,self.flapDepth,self.depth,self.width,
        ...
        if isGlue:
            ...
            self.canvas.rect(margin,self.flapDepth+margin,
                             self.depth-2*margin,self.width-2*margin,fill=True)
            self.canvas.setFillColorCMYK(0,0,0,1)
            self.canvas.rotate(-90)
            self.canvas.scale(1,-1)
            self.canvas.setFontSize(7)
            self.canvas.drawCentredString(-self.width/2-self.flapDepth,
                                           -self.depth/2-5,
            ...
            self.canvas.rotate(-90)
            self.canvas.drawImage(self.sideImage,
                                  ...
                                  preserveAspectRatio=self.preserveEndAspect, mask='auto')
        ...
        if not isGlue:
            self.canvas.arc(0,self.width,self.depth,2*self.flapDepth+self.width,
        ...
        if isTop:
            self.canvas.line(0,self.flapDepth,0,self.flapDepth+self.width)
        self.canvas.line(self.depth,self.flapDepth,self.depth,self.flapDepth+self.width)
        if isGlue:
            self.canvas.setDash()
        self.canvas.line(0,self.flapDepth,self.depth,self.flapDepth)
        self.canvas.line(0,self.width+self.flapDepth,self.depth,self.width+self.flapDepth)
    ...
    def drawSide(self,hasFlap=False,isGlue=False):
        ...
        if isGlue:
            ...
            self.canvas.setStrokeColorCMYK(0,0,0,0.1)
            margin = 7
            self.canvas.rect(margin,margin,self.height-2*margin,
                             self.depth-2*margin,fill=True)
            self.canvas.setFillColorCMYK(0,0,0,1)
            #self.canvas.rotate(-90)
            #self.canvas.scale(1,-1)
            self.canvas.setFontSize(8)
            self.canvas.drawCentredString(self.height/2,
                                          self.depth/2-5,
                                          "Cut Solid. Fold Dashed. Glue This.")
        ...
        if not hasFlap:
            ...
            self.canvas.arc(0,-arcWidth-extraFlap,arcWidth*2,arcWidth-extraFlap,180)
            self.canvas.line(arcWidth,-arcWidth-extraFlap,self.height-arcWidth,-arcWidth-extraFlap)
            self.canvas.arc(self.height-arcWidth*2,-arcWidth-extraFlap,self.height,arcWidth-extraFlap,-90)
    ...
    def drawFront(self):
        if self.frontImage:
            ...
            self.canvas.saveState()
            #self.canvas.rotate(90)
            self.canvas.drawImage(self.frontImage,margin,margin,
    ...
    def drawBack(self):
        if self.backImage:
            ...
            #     self.canvas.rotate(180)
            #self.canvas.rotate(90)
            self.canvas.drawImage(self.backImage,margin,margin,
                                  self.height-2*margin,self.width-2*margin,
                                  preserveAspectRatio=False, mask='auto')            
            self.canvas.restoreState()
        ...
        self.canvas.line(centre+fingerWidth/2.0,self.width,self.height,self.width)
        self.canvas.arc(centre-fingerWidth/2.0,
        ...
        self.canvas.wedge(centre-fingerWidth/2.0,
                          self.width-fingerWidth/2.0,
                          centre+fingerWidth/2.0,
                          self.width+fingerWidth/2.0,
                          startAng=180,
    ...
    def generate(self):
        ...
        #bottom glue flap
        self.canvas.saveState()
        self.canvas.translate(2*self.depth+2*self.height,self.depth-self.flapDepth)
...
if __name__ == '__main__':
    tuck = TuckBoxGenerator(6.7*cm,10.2*cm,1.6*cm,'7Wonders_age_2_3.pdf',
    ...
    tuck = TuckBoxGenerator(6.7*cm,10.2*cm,1.6*cm,
                            sideImage='7W_logo.png',
    ...
    tuck = TuckBoxGenerator(6.7*cm,10.2*cm,1.6*cm,'7Wonders_age_1_leaders.pdf',
                            sideImage='7W_logo.png',
    ...
    c = tuck.generate()
    tuck = TuckBoxGenerator(6.7*cm,10.2*cm,1.2*cm,
                            sideImage='7W_logo.png',
    ...
    tuck.generate()
    tuck.close()
    tuck = TuckBoxGenerator(6.8*cm,9.9*cm,2.4*cm,'Modern_Art.pdf',
    ...
    tuck.generate()
    tuck.close()
    tuck = TuckBoxGenerator(6.4*cm,8.8*cm,2*cm,'Claustrophobia_tuckbox.pdf',
    ...
    tuck.close()
    #monuments
    tuck = TuckBoxGenerator(4.2*cm,5.2*cm,2.3*cm,'tzolkin_monuments_starttile_tuckbox.pdf',
    ...
    c = tuck.generate()
    #starting tiles
    tuck = TuckBoxGenerator(2.7*cm,5.2*cm,2.6*cm,
                            sideImage='tzolkin_title.jpg',
                            endImage='tzolkin_title.jpg',
    ...
    tuck.close()
    #Age 1
    tuck = TuckBoxGenerator(4.2*cm,5.2*cm,2.5*cm,'tzolkin_ages_tuckboxes.pdf',
                            sideImage='tzolkin_title.jpg',
                            endImage='tzolkin_title.jpg',
    ...
    c = tuck.generate()
    #Age 2
    tuck = TuckBoxGenerator(4.2*cm,5.2*cm,3.1*cm,
                            sideImage='tzolkin_title.jpg',
                            endImage='tzolkin_title.jpg',
    ...
    #Istanbul
    tuck = TuckBoxGenerator(6.8*cm, 4.5*cm, 0.8*cm, 'istanbul_tuckbox.pdf',
